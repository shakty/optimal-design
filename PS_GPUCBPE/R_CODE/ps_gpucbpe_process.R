...
initialize_process <- function(posteriors, n_init, expParam1_range, expParam2_range, n_dim, sampling_method, n_players, n_rounds, 
							   n_samples, sobol, sampling, asymmetric, L, models_used, current_top_model=1){	
	# First: Call this function to get the initial dataframe and the corresponding information numbers for each point in the surface.
	datagrid <- expand.grid(A=seq(expParam1_range[1], expParam1_range[2], length.out=n_dim), PI=seq(expParam2_range[1], expParam2_range[2], length.out=n_dim))
	...
	for(row in 1:(nrow(history)-L+1)){
		...
		history[row,]$I <- getInfoNumber(a, p, sampling_method, posteriors, n_samples, asymmetric, sampling, models_used, n_players, n_rounds, current_top_model)
		print(sprintf("Initializing %02i %s seeds with %05s sampled histories per search ... Current Round is: %02i ... Current Time is: %s", n_init, sobol, n_samples, row, Sys.time()))
	}
